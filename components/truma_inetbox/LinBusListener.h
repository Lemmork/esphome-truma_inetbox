...
class LinBusListener : public PollingComponent, public uart::UARTDevice {
 ...
 protected:
  ...
  // HIER WAR DER FEHLER: Das Enum wurde angepasst, um zum alten Code zu passen
  enum ReadState {
    READ_STATE_BREAK,
    READ_STATE_SYNC,
    READ_STATE_SID,
  ...
  // Typ explizit gesetzt, damit die Zuweisungen funktionieren
  ReadState current_state_{READ_STATE_BREAK};

 private:
  uint32_t time_per_baud_{0};
  static constexpr uint8_t lin_break_length = 13;
  uint32_t time_per_lin_break_{0};
  static constexpr uint8_t frame_length_ = (8 + 1 + 2);
  ...
  QueueHandle_t lin_msg_queue_ =
      xQueueCreateStatic(TRUMA_MSG_QUEUE_LENGTH,
                         ...
                         lin_msg_static_queue_storage, &lin_msg_static_queue_);
  ...
  QueueHandle_t log_queue_ =
      xQueueCreateStatic(TRUMA_LOG_QUEUE_LENGTH,
                         ...
                         log_static_queue_storage, &log_static_queue_);
#endif
